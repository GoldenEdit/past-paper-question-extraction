...
def process_pdf_file(pdf_path, output_directory, paper):
    ...
    for i, image_path in enumerate(image_paths):
        ...
        ocr_results[f'page_{i + 2}'] = ocr_text

    saved_question_images = {}
    current_question = None
    for i, image_path in enumerate(image_paths):
        page_number = i + 2
        ocr_text = ocr_results.get(f'page_{page_number}', '')
        if ocr_text.strip().isdigit():
            current_question = int(ocr_text.strip())
    ...
    # Combine images for questions that span multiple pages
    combined_question_images = {}
    for question, image_paths in saved_question_images.items():
        if len(image_paths) > 1:
            combined_img = combine_images_vertically(image_paths)
            combined_image_path = os.path.join(output_directory, f'{paper}_q_{question}.png')
            combined_img.save(combined_image_path)
            combined_question_images[question] = combined_image_path
            for image_path in image_paths:
                os.remove(image_path)
        else:
            combined_question_images[question] = image_paths[0]
            os.rename(str(image_paths)[2:-2], os.path.join(output_directory, f'{paper}_q_{question}.png'))
